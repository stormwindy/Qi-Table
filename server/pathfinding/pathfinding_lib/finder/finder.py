...
class Finder(object):
    ...
    # NEW: takes table as an argument
    # uses table to to filter out neighbor cells
    def find_neighbors(self, grid, node, table, diagonal_movement=None):
        '''
        find neighbor, same for Djikstra, A*, Bi-A*, IDA*
        '''
        if not diagonal_movement:
            diagonal_movement = self.diagonal_movement
        neighbors = grid.neighbors(node, diagonal_movement=diagonal_movement)
        return grid.filter_with_safe_dist(neighbors, table)
    ...
    # NEW: takes table as an argument
    def find_path(self, table, start, end, grid):
        """
        ...
        while len(open_list) > 0:
            ...
            path = self.check_neighbors(table, start, end, grid, open_list)
            if path:
